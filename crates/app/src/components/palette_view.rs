...
/// Shared so the theme/model/subagent/project pickers all render rows the same way.
...
impl Workbench {
    /// The palette overlay: a query field over a filtered command list, painted above every pane.
    ...
    pub(crate) fn picker_popup(&self, cx: &mut Context<Self>) -> Option<gpui::AnyElement> {
        ...
        let panel = match picker {
            // `theme_list` brings its own filter field; adding a second one here would put two
