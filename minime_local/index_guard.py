...
def _sweep(filename: str) -> None:
    """Keep the newest [`KEEP`] copies and drop the rest."""
    directory = os.path.dirname(filename) or "."
    prefix = os.path.basename(filename) + MARK
    try:
        ours = sorted(name for name in os.listdir(directory) if name.startswith(prefix))
    except OSError:
        return
    for name in ours[: max(0, len(ours) - KEEP)]:
        _forget(os.path.join(directory, name))
