...
#: Where the conversation index is loaded, and deleted when it cannot be read. Patched always:
#: it has nothing to do with where the agent's code runs, and tying it to that switch is the
#: mistake §78 made with the subagent registry (docs §218).
_RUNTIME_DB_MODULE = "langgraph_runtime_inmem.database"
...
def _patch(module) -> None:
    ...
    if module.__name__ == _ARTIFACTS_MODULE:
        ...
        return
    if module.__name__ == _RUNTIME_DB_MODULE:
        from minime_local import index_guard

        index_guard.install(module)
        return
