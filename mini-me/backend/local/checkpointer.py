...
* **Boot cost grows with history.** Nothing the client does can help; the server is unpickling
  megabytes before it answers anything.
* **A failed load destroys everything.** ``checkpoint.py`` registers the dict with the flush loop
  *before* calling `load()`. When the load raises — a `ModuleNotFoundError` after an upgrade
  changed class definitions, say — the exception is swallowed and an **empty** dict is left
  registered. Ten seconds later `PersistentDict.sync()` pickles that empty dict over the real
  file, under a comment reading ``# atomic commit``.
...
**Why not Rust.** The cost here is unpickling megabytes and writing them back — serialisation and
I/O rather than computation — so there is no work for a faster language to do, and reaching Rust
from Python would mean PyO3 and a compiled wheel per platform: a new way for the install to fail.

**Why this is a file the server loads rather than a patch.** `langgraph.json` takes a
``checkpointer`` key naming an async context manager, and the desktop app already generates its
own copy of that config (`make_config.py`) rather than editing a checkout it does not own. So this
is configuration.
"""
...
async def checkpointer() -> AsyncIterator[object]:
    ...
    # never constructed, so the conversations it holds do not load.
    ...
    async with AsyncSqliteSaver.from_conn_string(database) as saver:
        ...
        if setup is not None:
            await setup()
        # Logged on **success**, and this is not decoration: a checkpointer that silently did not
        # take effect would look exactly like one that did, right up until someone noticed their
        # conversations were still slow to load.
        logger.warning("backend.local: conversations are stored in %s", database)
        yield saver
